...
namespace ACG_AUDIT
{
    internal static class Program
    {
        ...
        //private static readonly string agendadorPath = @"C:\Users\Gusta\Documents\Lidersis\ACGAUDIT2.0FORM\Sub-system-ACG\teste_codigo_csharp\bin\Release\net7.0\Agendador.exe";
        ...
        //private static readonly string executablePath = @"C:\Users\Gusta\Documents\Lidersis\ACGAUDIT2.0FORM\ACG AUDIT 2.0\bin\Release\net8.0\ACG AUDIT 2.0.exe";
        ...
        static void Main()
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    // Inicializar a variável para evitar problemas na serialização do JSON
                    ScreenSaverSettings? screenSaverSettings = null;

                    // Verificar se o arquivo existe antes de tentar acessá-lo
                    if (File.Exists(filePath))
                    {
                        screenSaverSettings = ScreenSaverService.GetScreenSaverSettings(filePath);
                        await UpdateStatusWithDelay("Informações de proteção de tela coletadas", timeDelay, loadingForm);
                    }
                    else
                    {
                        await UpdateStatusWithDelay("Arquivo Registry.pol não encontrado.", timeDelay, loadingForm);
                        screenSaverSettings = new ScreenSaverSettings(false, false, "", ""); // Instância vazia para evitar problemas no JSON
                    }
                    // END //
